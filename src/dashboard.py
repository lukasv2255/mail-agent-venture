...
def api_history(request: Request, page: int = 1, per_page: int = 50, filter: str = "all"):
    _check_token(request)
    history_file = _first_existing_path("logs/responder/responses.jsonl", "logs/responses.jsonl")
...
def api_sorter_history(request: Request, page: int = 1, per_page: int = 50, filter: str = "all"):
    _check_token(request)
    history_file = _first_existing_path("logs/sorter/sorter.jsonl", "logs/sorter.jsonl")
    if not history_file.exists():
        return {"items": [], "total": 0, "page": page, "per_page": per_page, "pages": 0}
    lines = history_file.read_text(encoding="utf-8").splitlines()
    items = []
